...
BEN_fun <- function(n_iter, burn_in, data, rho, thin, hyper.s=6, hyper.t=6, tune=0.005){
...
##################### fun_BEN + MCEM ###############################################################
func.ben <- function(n_iter, burn_in, data, mc_iter, tol = 1e-2, hyper.s=6, hyper.t=6, tune=0.005, init_rho = 0.5, thin =10, rho.select = 'mcem'){
  if (rho.select == 'mcem') rho <- tail(mcem(data, mc_iter, init_rho, tol = tol), 1)
  else rho <- rho.select
  
  return(BEN_fun(n_iter, burn_in, data, rho=rho, hyper.s, hyper.t, tune, thin))
}
